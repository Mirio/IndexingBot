...
def start_helper(message):
    ...
    try:
        if "it" in message.from_user.language_code:
            desc = (
                "Ciao %s, questo bot e' stato creato al fine di 'unificare'"
                ...
                "Lasciaci una review:"
                " https://storebot.me/bot/indexingbot" % message.from_user.first_name
            )
            markup.add(
                telebot.types.InlineKeyboardButton(
                    "Richiedi l'aggiunta del tuo gruppo", callback_data="add_helper"
                )
            )
            markup.add(
                telebot.types.InlineKeyboardButton(
        ...
        desc = (
            "Ciao %s, this bot was created with the purpose to index "
        ...
        markup.add(
            telebot.types.InlineKeyboardButton("Search Group", callback_data="search")
        )
...
def showlist(message):
    ...
    msgout = ""
    res = algolia_index.search("", {"filters": "enabled = 1", "hitsPerPage": 500})
...
def waitlist(message):
    ...
    algolia_index = algolia.init_index("groups")
    msgout = ""
    res = algolia_index.search("", {"filters": "enabled = 0", "hitsPerPage": 500})
    for group in res["hits"]:
        msgout += "--\nName: %s\nDescription: %s\nLink: %s\n" % (
            group["name"],
            group["desc"],
    ...
    try:
        ...
        if len(msg_split) <= 2:
            ...
            res = algolia_index.search(
                cleantext(msg_split[1]), {"filters": "enabled = 1", "hitsPerPage": 500}
            )
            for group in res["hits"]:
                try:
                    if "it" in message.from_user.language_code:
            ...
            for text in splitted_text:
                bot.send_message(
                    message.from_user.id, (text), disable_web_page_preview=True
                )
            return
        else:
            raise KeyError
                ...
                try:
                    if "it" in message.from_user.language_code:
                        bot.send_message(
                            ...
                            ("Utilizza un url t.me valido." " (usa https:// prima)"),
                        )
                        return
                    else:
                        raise TypeError
                    ...
                    bot.send_message(
                        message.from_user.id,
                        ("Use the t.me format for URL." " (use schema https)"),
                    )
                    return
            ...
            for group in res["hits"]:
                if group["url"] == url:
                    try:
                        if "it" in message.from_user.language_code:
                            bot.send_message(
                                message.from_user.id, ("Gruppo gia' presente.")
                            )
                            return
                        else:
                            raise TypeError
                        ...
                        bot.send_message(
                            message.from_user.id, ("Group already present.")
                        )
                        return
            ...
            for group in res["hits"]:
                if group["name"] == name:
                    try:
                        if "it" in message.from_user.language_code:
                            bot.send_message(
                                message.from_user.id, ("Nome gia' presente.")
                            )
                            return
                        else:
                            raise TypeError
                    except TypeError:
                        bot.send_message(message.from_user.id, ("Name already taken."))
                        return
